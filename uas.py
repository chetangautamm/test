...
def create_ns_instance(url, token, nsdId, vimAccountId):
    ...
    payload = json.dumps({ "nsName": "uas", "nsdId": f"{nsdId}", "vimAccountId": f"{vimAccountId1}", "nsDescription": "Generated by Automation" })
    response = requests.request("POST", url, headers=headers, data=payload, verify=False)
    print(response.text)
    return response.json()['id']
...
vnfpackage = "uas-knf.tar.gz"
nspackage = "uas-kns.tar.gz"
